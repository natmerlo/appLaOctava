<div>
      <div wire:click="VerForm()" title="Revisión de datos" class="p-1 h-[2rem] w-[2rem] hover:scale-125 transition-transform duration-200 hover:cursor-pointer">
      ...
      <section class="modal_container z-[999]" x-cloak x-show="verForm = $wire.verForm" x-transition.duration.0ms>
            <div class="panel-modal-1">
                  
                  <div class="gr01 h-full">
                        <div class="gr01-C1 flex justify-center items-center">
                              <img class="w-[19rem]" src="{{ asset('img/recetas/' . $dtosReceta->pathImg) }}" alt="Receta">
                        </div>
                        <div class="gr01-C2 flex justify-center">
                              <div style="overflow-y: auto; max-height: 200px;">
                                    <ul class="list-disc list-inside order-3 text-white text-left">
                        ...
                        <div class="gr01-C3">
                              <h2 class="subtitulo eucrosia text-white">{{ $dtosReceta->nombre }}</h2>
                        </div>
                        <div class="gr01-C4 pt-4">
                        ...
                        <div class="gr01-C5">
                              <div style="overflow-y: auto; max-height: 18rem;">
                                    @foreach ($dtosPrepar as $it)
                                          @if ($it->tagHtml == 't')
                                                <h2 class="text-left mt-3 mb-3 text-white">{{ $it->parrafo }}</h2>
                                          @endif
                                          @if ($it->tagHtml == 'p')
                                                <p class="text-left mb-3 text-white">{{ $it->parrafo }}</p>
                                          @endif
                                    @endforeach
                              </div>
                        </div>
                  ...
                  <div class="flex justify-end pb-2">
                        <div wire:click="CerrarForm()" class="absolute top-3 right-3 w-8 h-8 hover:scale-105 flex cursor-pointer items-center justify-center border-[2px] border-white rounded-full">
                              <svg class="font-extrabold w-5 h-5 text-white" data-slot="icon" fill="none" stroke-width="1.5" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
